<x-app-layout>
    <x-slot name="header">
        ...
        @can('create sales')
        <a href="{{route('sale.create')}}" class="bg-slate-700 text-sm rounded-md px-3 py-3 text-white">Create</a>
        @endcan
    </div>
    </x-slot>
                ...
                <tbody class="bg-white">
                    ...
                    <tr class="border-b">
                        ...
                        <td class="px-3 py-3 text-center">
                            @can('edit sales')
                            <a href="{{route('sale.edit',$list->id)}}" class="bg-slate-700 text-sm rounded-md px-3 py-3 text-white">Edit</a>
                            @endcan
                            @can('delete sales')
                            <a href="javascript:void(0)" onclick="deleteSale({{$list->id}})" class="bg-red-500 text-sm rounded-md px-3 py-3 text-white">Delete</a>
                            @endcan
                        </td>
                    </tr>
                    @endforeach
